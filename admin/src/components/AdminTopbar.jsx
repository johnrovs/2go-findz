...
function buildBreadcrumbs(pathname) {
  ...
  return ['Dashboard', ...segments.map((segment) => BREADCRUMB_LABELS[segment] ?? segment)];
}
...
function isBuyingGuideEditorPath(pathname) {
  return /^\/buying-guides\/(new|\d+)$/.test(pathname);
}
...
function AdminTopbar({ onMenuClick }) {
  ...
  if (location.pathname === '/' || isBuyingGuideEditorPath(location.pathname)) return null;
