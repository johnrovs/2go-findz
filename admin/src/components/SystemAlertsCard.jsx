    ...
    <div className="flex h-full flex-col rounded-card border border-slate-200 bg-white p-5 shadow-card">
      ...
      {!hasAlerts ? (
        ...
        <ul className="space-y-1">
          {draftProductCount > 0 && <AlertRow to="/products">{draftProductMessage}</AlertRow>}
          {draftGuideCount > 0 && <AlertRow to="/buying-guides">{draftGuideMessage}</AlertRow>}
          {emptyCategoryCount > 0 && <AlertRow to="/categories">{emptyCategoryMessage}</AlertRow>}
        </ul>
      )}
    </div>
  );
